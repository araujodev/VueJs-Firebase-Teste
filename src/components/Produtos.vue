<template>
  <div>
    <h3> <i class="small material-icons">add_shopping_cart</i> {{ labelSpace }} </h3>
    <div class="row">
      <form class="col s12">
        <div class="row">
        ...
        <div class="row">
          ...
          <div class="input-field col m6 s12">
            ...
            <label v-bind:class="{ active: labelIsActive }" for="estoque">Quantidade Atual no Estoque</label>
          </div>
        </div>
        <div class="row">
          <a @click.prevent="adicionarOuCriarProduto" class="waves-effect waves-light btn">{{ labelSpace }}</a>
        </div>
      </form>
    </div>

    <h3> <i class="small material-icons">add_shopping_cart</i>  Produtos</h3>
    <div class="row">
      <div v-for="produto in produtos" :key="produto.produto_id" class="col s12 m6">
        <div class="card-panel grey lighten-4">
          <h5 class="gray-text"> {{ produto.nome }} </h5>
          <p class="gray-text"> <b>Preço de Venda: </b>  R$ {{ produto.preco }} </p>
          <p class="gray-text"> <b>Estoque: </b> {{ produto.estoque }} <b>unidades.</b> </p>
          <p class="gray-text"> <b>Cód. Produto: </b> {{ produto.produto_id }} </p>
          <p>
            <a @click.prevent="editarProduto(produto.produto_id)" class="waves-effect blue lighten-1 btn"><i class="material-icons left">create</i> Editar</a>
            <a @click.prevent="removerProduto(produto.produto_id)" class="waves-effect red lighten-1 btn"><i class="material-icons left">delete</i> Remover</a>
          </p>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    geradorUnicoCodigoProduto () {
      this.produtoDados.produto_id = 'cod_' + Math.random().toString(36).substr(2, 9)
    },
    recuperarProdutos () {
    ...
    adicionarOuCriarProduto () {
      ...
      .then(querySnapshot => {
        ...
        if(contador > 0){
          //Editar Antigo
          db.collection('Produtos').where('produto_id', '==', this.produtoDados.produto_id)
            .get()
            .then(querySnapshot => {
              querySnapshot.forEach(doc => {
                doc.ref.set({
                  ...
                  this.recuperarProdutos()
                  this.produtoDados = {produto_id: '', nome: '', preco: '', estoque: ''}
                  this.geradorUnicoCodigoProduto()
          ...
          //Criar Novo
          db.collection('Produtos').add(this.produtoDados).then(() => {
            this.produtoDados = {produto_id: '', nome: '', preco: '', estoque: ''}
            this.geradorUnicoCodigoProduto()
            this.recuperarProdutos()
            this.labelIsActive = false;
    ...
    editarProduto (produtoId) {
      ...
      .get()
      .then(querySnapshot => {
        let produto = {produto_id: '', nome: '', preco: '', estoque: ''}
        querySnapshot.forEach(doc => {
          produto = doc.data()
        })
